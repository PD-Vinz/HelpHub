    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function showSuggestions(query) {
            if (query.length >= 4) {
                fetch(`suggest.php?query=${encodeURIComponent(query)}&user=${encodeURIComponent(identity)}`)
                    .then(response => {
                        if (!response.ok) {
        ...
        document.addEventListener('click', function(event) {
            if (!suggestions.contains(event.target) && event.target !== fnameInput && event.target !== emailInput) {
                hideSuggestions();
            }
        });
    });
    ...
    document.getElementById('issueForm').addEventListener('submit', function (event) {
        ...
        const pastedImageData = document.getElementById('pasted-image-data');
        const uploadOption = document.querySelector('input[name="uploadOption"]:checked').value;
    
        // Prevent default submission to handle FormData
        event.preventDefault();
    
        // Create a FormData object
        const formData = new FormData(this);
    
        if (uploadOption === 'paste' && pastedImageData.value) {
            // Convert base64 to Blob
            const base64Data = pastedImageData.value.split(',')[1]; // Remove the "data:image/*;base64," prefix
            const contentType = pastedImageData.value.match(/^data:(.*);base64,/)[1];
            ...
            // Append the Blob to the FormData as a file
            formData.append('image', blob, 'pasted-image.png');
        }// else if (uploadOption === 'upload' && !fileInput.files.length) {
...
//--- Selection ---//
function handleUploadOptionSelection() {
    const uploadContainer = document.getElementById('upload-container');
    const pasteContainer = document.getElementById('paste-container');
    const uploadOptionInputs = document.querySelectorAll('input[name="uploadOption"]');
    const fileInput = document.getElementById('file-input');
    const pastedImageData = document.getElementById('pasted-image-data');
    const pastedPreview = document.getElementById('pastedPreview');

    uploadOptionInputs.forEach(option => {
        option.addEventListener('change', () => {
            if (option.value === 'upload') {
                uploadContainer.style.display = 'block';
                pasteContainer.style.display = 'none';

                // Clear paste-related fields
                pastedImageData.value = '';
                pastedPreview.style.display = 'none';
            } else if (option.value === 'paste') {
                uploadContainer.style.display = 'none';
                pasteContainer.style.display = 'block';

                // Clear file input
                fileInput.value = '';
                const fileMessage = uploadContainer.querySelector('.file-message');
...
function handleImagePaste() {
    ...
    pasteArea.addEventListener('paste', (event) => {
        ...
        for (let i = 0; i < items.length; i++) {
            if (items[i].type.startsWith('image/')) {
                ...
                if (blob.size > maxFileSize) {
                    pasteError.textContent = 'Pasted image exceeds 6 MB limit.';
                    return;
                }
                ...
                reader.onload = (e) => {
                    ...
                    // Display the preview
                    pastedPreview.src = base64Data;
                    pastedPreview.style.display = 'block';

                    // Populate the hidden input with base64 image data
                    pastedImageData.value = base64Data;
                };
        ...
        if (!foundImage) {
            pasteError.textContent = 'No valid image found in clipboard.';
        } else {
